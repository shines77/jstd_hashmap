...
#define USE_JSTD_ROBIN_HASH_MAP     0
...
static void report_result(char const * title, double ut, double mlf, std::size_t iters,
    ...
    printf("%-35s %8.2f ns  mlf=%0.3f  %s\n", title, (ut * 1000000000.0 / iters), mlf, heap);
#else
  #if USE_CTOR_COUNTER
    printf("%-35s %8.2f ns  (%8" PRIuPTR " hashes, %8" PRIuPTR " copies, %8" PRIuPTR " ctor) mlf=%0.3f %s\n",
           title, (ut * 1000000000.0 / iters),
           g_num_hashes, g_num_copies, g_num_constructor,
           mlf, heap);
  #else
    printf("%-35s %8.2f ns  (%8" PRIuPTR " hashes, %8" PRIuPTR " copies) mlf=%0.3f %s\n",
           title, (ut * 1000000000.0 / iters),
