...
#define ROBIN_V3_USE_HASH_POLICY       0
#define ROBIN_V3_USE_SWAP_TRAITS       1
...
public:
    ...
    typedef v3::robin_hash_map_slot_policy<Key, Value, slot_type>
                                                    slot_policy_t;
    ...
    static_assert(false, "jstd::v3::robin_hash_map<K,V> required Intel AVX2 or heigher intrinsics.")
...
public:
    ...
    static const char * name() {
        return "jstd::v3::robin_hash_map<K, V>";
    }
    ...
    mapped_type & at(const key_type & key) {
        ...
        if (slot != this->last_slot()) {
            ...
            throw std::out_of_range("std::out_of_range exception: jstd::v3::robin_hash_map<K,V>::at(key), "
    ...
    const mapped_type & at(const key_type & key) const {
        ...
        if (slot != this->last_slot()) {
            ...
            throw std::out_of_range("std::out_of_range exception: jstd::v3::robin_hash_map<K,V>::at(key) const, "
                                    "the specified key is not exists.");
        }
    }
