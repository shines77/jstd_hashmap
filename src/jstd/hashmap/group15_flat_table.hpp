...
public:
    ...
    using iterator       = jstd::flat_map_iterator15<this_type, value_type, kIsIndirectKV>;
    using const_iterator = jstd::flat_map_iterator15<this_type, const value_type, kIsIndirectKV>;

    using locator_t = jstd::flat_map_locator15<this_type, kIsIndirectKV>;
...
#pragma warning(pop) /* C4800 */
    ...
    JSTD_FORCED_INLINE
    void no_grow_unique_transfer_insert(group15_flat_table * other, slot_type * old_slot) {
        assert(old_slot != nullptr);
        locator_t locator = this->no_grow_unique_insert(old_slot->get_key());
        slot_type * new_slot = locator.slot();
        assert(new_slot != nullptr);
